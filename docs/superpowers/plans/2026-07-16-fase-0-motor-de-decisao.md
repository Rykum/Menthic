...
- **Constantes do modelo (doc 10 A.1), exatas:** `c0=-1.0`, `cP=2.5`, `cF=2.0`,
  `A=0.6`, `f0base=0.10`, `kD=0.08`, `kWork=3.0` (escala de trabalho, calibrada —
  ver Task 5 e Notas de calibração), `N=2000` (outer `K=200` × inner `m=10`),
...
class ModelConstants {
  ...
  static const double kWork = 3.0; // escala de trabalho (calibrada, doc 10 A.6)
}
...
**Resolvido na execução (Task 5):** o teste de aceitação detectou que
`taxa_progresso = p0·energia (≈0,28/h)` tornava a tarefa-exemplo inatingível
(conclusão ~0% para todo débito de sono). Introduzida a escala `kWork` (doc 10 A.6)
